...
export const deleteReview = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error occurred while deleting user history" });
  }
};
export const updateReview = async (req: Request, res: Response) => {
  try {
    const reviewInfo = req.body;
    const { id, retting, productId, productName, review, categoryName, email } =
      reviewInfo;
    ...
    try {
      if (!id || result?.matchedCount === 0) {
        // If no matching review was found, create a new one
        ...
        // Update the new ID in the response
        result = { matchedCount: 1, insertedId: newReview._id };
      }
    } catch (e) {
      console.error("Error creating review:", e);
      return res
        .status(400)
        .send({ message: "please give ratings and Review also" });
    }

    if (result.matchedCount === 1) {
      // Update the rating in the Product collection
      ...
      return res
        .status(400)
        .send({ message: "Failed to update or create review" });
    }
  } catch (err) {
    console.error("Error updating review:", err);
    return res
      .status(500)
      .send({ message: "Error occurred while updating the review" });
  }
};
