...
export const updateProductInfo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await Order.updateMany(
      { "orderProducts._id": id }, // Corrected field reference
      {
        $set: {
          "orderProducts.$[elem].productName": productName,
          "orderProducts.$[elem].price": price,
          "orderProducts.$[elem].oldPrice": oldPrice,
          "orderProducts.$[elem].subcategoryName": subcategoryName,
          "orderProducts.$[elem].categoryName": categoryName,
          "orderProducts.$[elem].productDetails": productDetails,
          "orderProducts.$[elem].submissionDate": submissionDate,
        },
      },
      { arrayFilters: [{ "elem._id": id }], session } // Corrected array filter condition
    );

    // Update EventSubmission Collection
    await EventSubmission.updateMany(
      { eventUserId: id }, // Corrected field reference
      {
        $set: {
          submissionDate
        },
    ...
    await session.abortTransaction(); // Ensure transaction is aborted on failure
    session.endSession();
    res.send({ message: "custom error" });
  }
};
...
export const Rattings = async (req: Request, res: Response) => {
  try {
    ...
    ratting.forEach((review) => {
      const rating = Number(review.retting);
